...
// Drivetrain Section (BVR1)
// Motor brackets, Hub motors, Wheel alignment

= Drivetrain

The BVR1 uses skid-steer drive: four independent hub motors, one in each wheel. To turn, the left and right sides spin at different speeds (or opposite directions). It's the same principle as a tank or a Roomba.
...
BVR1 uses 8" hub motors (larger than BVR0's 6.5" hoverboard motors) for improved ground clearance and obstacle handling. These mount via custom brackets rather than directly to the frame, making wheel service easier.
...
The BVR1 motor brackets use a bicycle-style design: an L-shaped aluminum bracket bolts to the frame, and the motor axle clamps into the bracket's slot. This allows quick wheel removal without disturbing the frame.
...
#figure(
  cetz.canvas({
    ...
    // Motor mounting holes (42mm square pattern for 8" motors)
...
#grid(
  ...
  [
    *CAD Files:*
    Available at `bvr/cad/exports/`:
    - `motor_mount_8in.stl` (3D model)
    - `motor_mount_plate_8in.stl`
    - `motor_mount_tab_8in.stl`

    #v(0.3em)
    #text(size: 7pt, fill: gray)[SendCutSend accepts DXF/STEP files for laser cutting and bending.]
  ]
)

#note[
  The 42mm hole pattern fits standard 8" hub motors. Verify your motor's bolt pattern before cutting.
]
