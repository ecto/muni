...
async fn main() -> Result<()> {
    ...
    // Resolve rover_id: CLI > config
    let rover_id = config.identity.rover_id.clone();
    ...
    let metrics_enabled = !args.no_metrics && config.metrics.enabled;
    if metrics_enabled {
        let metrics_config = MetricsConfig {
            enabled: true,
            endpoint: args.metrics_endpoint.clone().unwrap_or(config.metrics.endpoint.clone()),
            interval_hz: args.metrics_hz.unwrap_or(config.metrics.interval_hz),
            rover_id: rover_id.clone(),
        };
    ...
    // Priority: CLI args > config file
    let discovery_enabled = !args.no_discovery && config.discovery.enabled;
    if discovery_enabled {
        let discovery_endpoint = args.discovery_endpoint.clone()
            .unwrap_or(config.discovery.endpoint.clone());
        let discovery_rover_id = config.discovery.rover_id.clone()
            .unwrap_or(rover_id.clone());
        let discovery_rover_name = config.discovery.rover_name.clone()
            .unwrap_or_else(|| discovery_rover_id.replace("bvr-", "Beaver-").replace("frog-", "Frog-"));

        let discovery_config = DiscoveryConfig {
            enabled: true,
            endpoint: discovery_endpoint.clone(),
            heartbeat_secs: config.discovery.heartbeat_secs,
            rover_id: discovery_rover_id,
            rover_name: discovery_rover_name,
            ws_port: config.discovery.ws_port,
            ws_video_port: config.discovery.ws_video_port,
        };
